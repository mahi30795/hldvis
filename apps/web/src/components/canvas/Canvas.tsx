...
export const Canvas: React.FC = () => {
  ...
  const onConnect = useCallback((connection: Connection) => {
    const edge: DesignEdge = {
      ...
      type: 'dataflow', source: connection.source ?? '', target: connection.target ?? '',
    };
    storeAddEdge(edge);
  }, [storeAddEdge]);
  ...
  const onDrop = useCallback((event: React.DragEvent) => {
    ...
    const newNode: DesignNode = { id: `node-${nodeIdCounter++}`, type: componentType, position, data: { componentType, label } };
    addNode(newNode);
  }, [addNode]);
  ...
  return (
    <div ref={reactFlowWrapper} className="flex-1 h-full">
      <ReactFlow
        ...
        onConnect={onConnect} onInit={(i) => { reactFlowInstance.current = i; }}
        onDragOver={onDragOver} onDrop={onDrop} nodeTypes={nodeTypes} edgeTypes={edgeTypes}
      ...
      >
        ...
        <MiniMap nodeStrokeWidth={3} zoomable pannable maskColor="var(--rf-minimap-mask)" />
        <Controls />
      </ReactFlow>
    </div>
  );
